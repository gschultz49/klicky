...
export const Klicky = (dataSelector: string) => {
  ...
  textBox.addEventListener(
    "keydown",
    (event) =>
      (output.textContent = `Key board event occurred: You pressed "${
        event.key
      }".${click_arr.push(event)}`)
  );
  ...
  let dateTime = (date: Date) => {
    ...
    console.log(
      `Click event occurred at: ${month}/${day}/${year} ${hours}:${minutes}:${seconds}`
    );
    return msg;
  };
};
